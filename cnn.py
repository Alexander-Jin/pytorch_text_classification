...
parser.add_argument('-epochs', type=int, default=50, help='number of epochs for train [default: 300]')
...
parser.add_argument('-kernel-num', type=int, default=40, help='number of each kind of kernel')
parser.add_argument('-kernel-sizes', type=str, default="1,2,3,4,5",
                    help='comma-separated kernel size to use for convolution')
parser.add_argument('-device', type=int, default=0, help='device to use for iterate data, -1 mean cpu [default: -1]')
parser.add_argument('-no-hhy', action='store_true', default=False, help='fdsu')
...
class CNN(nn.Module):
    def __init__(self, args):
        ...
        self.conv1_bn = nn.ModuleList([nn.BatchNorm2d(Co) for K in Ks])
        self.fc1 = nn.Linear(len(Ks) * Co, len(Ks) * Co // 3)
        self.fc2 = nn.Linear(len(Ks) * Co // 3, C)
        self.fc1_bn = nn.BatchNorm1d(len(Ks) * Co // 3)
    ...
    def forward(self, x):
        ...
        for i, bn in enumerate(self.conv1_bn):
            x[i] = bn(x[i])
...
torch.save({'state_dict': model.state_dict(), 'optimizer' : optimizer.state_dict()}, "checkpoint")
